...
--as-parquetfile --target-dir /etl/movielens/user_upserts --query 'SELECT user.id, user.age, user.gender, occupation.occupation, zipcode, last_modified FROM user JOIN occupation ON (user.occupation_id = occupation.id) WHERE $CONDITIONS'
